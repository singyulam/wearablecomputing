 ...
 * Moved calibration code into respective functions, implemented on all three axes
 * Added control of transistors
 */
...
int lastChangeTimeX = 0;             // last significant change stored to be compared to current time
int lastChangeTimeY = 0;             // last significant change stored to be compared to current time
int lastChangeTimeZ = 0;             // last significant change stored to be compared to current time

boolean initialCalibrate = true;     // boolean for first calibration
boolean xCalibrate = false;          // boolean for x-axis calibration
boolean yCalibrate = false;          // boolean for y-axis calibration
boolean zCalibrate = false;          // boolean for z-axis calibration
...
void calibrateX () {
  ...
  if (xCalibrate && (currentTime-lastChangeTimeX>1000) && (xAverage-xLevelReadingNew < 100) ) {
    xLevelReading = xLevelReadingNew;
    xCalibrate = false;
  }
  if (xCalibrate && currentTime-lastChangeTimeX>1100) {
    xCalibrate = false;
  }  
}

void calibrateY () {
  if (!yLevel && !yCalibrate) {
    yLevelReadingNew = yAverage;
    lastChangeTimeY = millis();
    yCalibrate = true;
  }
  if (yCalibrate && (currentTime-lastChangeTimeY>1000) && (yAverage-yLevelReadingNew < 100) ) {
    yLevelReading = yLevelReadingNew;
    yCalibrate = false;
  }
  if (yCalibrate && currentTime-lastChangeTimeY>1100) {
    yCalibrate = false;
  }  
}

void calibrateZ () {
  if (!zLevel && !zCalibrate) {
    zLevelReadingNew = zAverage;
    lastChangeTimeZ = millis();
    zCalibrate = true;
  }
  if (zCalibrate && (currentTime-lastChangeTimeZ>1000) && (zAverage-zLevelReadingNew < 100) ) {
    zLevelReading = zLevelReadingNew;
    zCalibrate = false;
  }
  if (zCalibrate && currentTime-lastChangeTimeZ>1100) {
    zCalibrate = false;
  }
}  
